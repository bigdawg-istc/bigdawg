...
int Scidb2Postgres::scidb2postgres(FILE* inFile, std::vector<boost::shared_ptr<Attribute> > &attributes, FILE* outFile) {
    ...
    do {
        ...
        for (std::vector<boost::shared_ptr<Attribute> >::iterator it=attributes.begin(); it != attributes.end(); ++it) {
            ...
            if (it == attributes.begin()) {
                ...
                Postgres::writeColNumber(outFile,static_cast<uint16_t>(attributes.size()));
            }
