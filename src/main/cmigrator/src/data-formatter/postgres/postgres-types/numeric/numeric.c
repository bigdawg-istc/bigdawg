...
#include "../../../data-formatter/postgres-types/numeric/pgtypes_error.h"
#include "../../../data-formatter/postgres-types/numeric/pgtypes_numeric.h"
...
{
    char	   * newSpace = (char *) calloc(1L, size);
...
PGTYPESnumeric_div(numeric *var1, numeric *var2, numeric *result)
{
	...
	std::memcpy(&(divisor[1].digits[1]), var2->digits, ndigits_tmp - 1);
	...
	std::memcpy(dividend.digits, var1->digits, var1->ndigits);
	...
	for (ri = 0; ri <= res_ndigits; ri++)
	{
		...
		while (guess > 0)
		{
			if (divisor[guess].buf == NULL)
			{
				...
				std::memcpy(&divisor[guess], &divisor[1], sizeof(numeric));
...
/*##############################################################################*/
/* from numeric.c : src/backend/utils/adt/numeric.c
 *
