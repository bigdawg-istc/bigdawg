...
public class Planner {
	 ...
	 * 
	 * 1. Planner receives the query and calls a monitor function to deliver
	 * alternative subqueries; 2. Monitor compiles an performance information
	 * and calls a planner function to deliver it; 3. Planner comes up with a
	 * series of subqueries that represents the best cross-island join-order and
	 * call executor to execute each sub-query; 4. Executor finds the best way
	 * to do each sub-query, execute everything locally, call the migrator to
	 * move finished cross-island intermediate results, and repeat; 5. Executor
	 * calls planner to deliver the final result back to user. if a query engine
	 * or a migrator is down and beyond localized recovery, executor and
	 * migrator should report to user.
	 * 
	 * Note: Let's assume that each query has a serial number or identifier
	 * associated with it.
	 * 
	 * UPDATE, IMPORTANT: To have this thing going, we need a PostgreSQL
	 * database named "bigdawg" to house the Catalog. Also, to make it work,
	 * we'll need to run CatalogInitiator.createSchemaAndRelations(Catalog cc)
	 * to create the schema, relation, etc. Only then we can start using this
	 * Planner.
	 * 
	 * Also, currently we assume that the query we run is wrapped in
	 * "bdrel(...)", and runs on a single query, and no intermediate result.
	 * 
	 * //
	 */

	private static LinkedList<ArrayList<String>> queryQueue = new LinkedList<ArrayList<String>>(); // queries
																									// to
																									// be
																									// executed
	private static LinkedList<Integer> serialQueue = new LinkedList<Integer>(); // used
																				// to
																				// store
																				// serial
																				// number
	...
	public static void getGetPerformanceAndPickTheBest(String userinput) throws Exception {
		...
		// Today there are no multiple plans
		QueriesAndPerformanceInformation qnp = Monitor.getBenchmarkPerformance(permuted); // TODO
																							// RETURN
																							// SOMETHING?

		// does some magic to pick out the best query, store it to the query
		// plan queue
		maxSerial += 1;
