...
public class CatalogModifier {
	 ...
	 * @throws Exception
	 */
	public static void addEngine(Catalog cc, String newEngine, String newURL, int newPort, String newProperty) throws Exception {
		
		// check if cc is connected and length are correct 
		CatalogUtilities.checkConnection(cc);
		CatalogUtilities.checkLength(newEngine, 15);
		CatalogUtilities.checkLength(newURL, 40);
		CatalogUtilities.checkLength(newProperty, 30);
		...
		if ( !rs.next() ) {
        	...
        	cc.execNoRet("INSERT INTO catalog.engines (eid, name, url, port, connection_properties) "
	        			+ "VALUES ("+ newpos 		+   ", "
	    					+ "\'"	+ newEngine 	+ "\', "
							+ "\'"	+ newURL 		+ "\', "
	    							+ newPort 		+ 	", "
