...
/**
 * Supplements PlanParser for things that are not captured in psql explain statement
   This code walks the query parse tree of a SQL statement
 and records the snippets that would otherwise be excluded
 
 ** Ony run this on PlainSelect statements, not ones including WITH/CTEs

 keeps track of:
   attribute aliases (and any corresponding table aliases)
   OVER clause for windowed aggregates
   ORDER BY direction (ASC | DESC)
   
 **/

public class SQLHandler implements SelectVisitor, OrderByVisitor, SelectItemVisitor, FromItemVisitor, PivotVisitor {
     ...
     */
    public SQLHandler(ExpressionVisitor expressionVisitor, StringBuilder buffer, SQLTableExpression tev) {
        this.buffer = buffer;
        this.expressionVisitor = expressionVisitor;
        this.supplementVariables = tev;
    }

    // reset once per SELECT / common table expression
...
//        }
    ...
    public void visit(SelectExpressionItem selectExpressionItem) {
        ...
        if (selectExpressionItem.getAlias() != null) {
            ...
            supplementVariables.addAlias(selectExpressionItem);
    ...
    public void deparseOrderBy(boolean oracleSiblings, List<OrderByElement> orderByElements) {
        ...
        supplementVariables.setOrderBy(orderByElements);
    ...
    public void deparseJoin(Join join) {
    	...
    	supplementVariables.addJoin(join);
        if (join.isSimple()) {
            buffer.append(", ");
        } else {
