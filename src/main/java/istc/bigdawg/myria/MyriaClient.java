...
public class MyriaClient {
	...
	public static String startQuery(String query)
			throws IOException, MyriaException {
		// System.out.println("Execute Myria query.");
		String finalURI;
		try {
			finalURI = String.format(
					"http://%s:%s/execute?language=myrial&query=%s", HOST, PORT,
					URLEncoder.encode(query, Constants.ENCODING));
	...
	public static String waitForCompletion(String jsonData) throws IOException {
		...
		do {
			...
			log.debug("Return code for waitForCompletion: " + returnCode);
	...
	public static void main(String[] arg) {
		...
		try {
			result = execute(
					"T1 = empty(x:int); T2 = [from T1 emit $0 as x]; store(T2, JustX);");
