...
public class FromPostgresToSciDB implements FromDatabaseToDatabase {
	...
	public MigrationResult migrateSingleThreadCSV(
		...
		try {
			try {
				...
				e.printStackTrace();
				return MigrationResult.getFailedInstance(
						errMessage + " Problem with opening a file: "
								+ csvFilePath + " for writing.");
			}
			Long extractedRows;
			try {
				extractedRows = copyManagerPostgreSQL
			...
			try {
				columnsMetaData = new PostgreSQLHandler(connectionFrom)
			...
			String typesPattern = SciDBHandler
					.getTypePatternFromPostgresTypes(columnsMetaData);
			...
			try {
				...
				e.printStackTrace();
				return MigrationResult.getFailedInstance(errMessage
						+ " Conversion from csv to scidb format failed! "
						+ e.getMessage());
			}
			// save the disk space
			...
			try {
				...
				return MigrationResult.getFailedInstance(
						errMessage + " Final data loading to SciDB failed! "
								+ e.getMessage());
			}
			return new MigrationResult(extractedRows, null,
	...
	public String loadDataToSciDB(SciDBConnectionInfo conTo, String arrayTo,
			String dataFile)
					throws IOException, InterruptedException, SciDBException {
		InputStream resultInStream = RunShell.executeAQLcommandSciDB(
				conTo.getHost(), conTo.getPort(), conTo.getBinPath(),
				"load " + arrayTo + " from '" + dataFile + "'");
