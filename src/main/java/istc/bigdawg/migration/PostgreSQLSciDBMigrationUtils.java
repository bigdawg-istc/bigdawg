...
public class PostgreSQLSciDBMigrationUtils {
	...
	/* log */
	private static Logger log = Logger.getLogger(PostgreSQLSciDBMigrationUtils.class);
	...
	public static boolean isFlatArray(SciDBArrayMetaData scidbArrayMetaData,
			PostgreSQLTableMetaData postgresqlTableMetaData) throws MigrationException {
		...
		if (areAttributesSameAsColumns(scidbArrayMetaData, postgresqlTableMetaData)) {
			return true;
		}
		return false;
	}
	 ...
	 * @throws MigrationException
	 */
	public static boolean areAttributesSameAsColumns(SciDBArrayMetaData scidbArrayMetaData,
			PostgreSQLTableMetaData postgresqlTableMetaData) throws MigrationException {
		...
		return areSciDBColumnsSameAsPostgresColumns(scidbArrayMetaData, postgresqlTableMetaData, scidbAttributesOrdered,
	 ...
	 * @throws MigrationException
	 */
	public static boolean areDimensionsAndAttributesSameAsColumns(SciDBArrayMetaData scidbArrayMetaData,
			PostgreSQLTableMetaData postgresqlTableMetaData) throws MigrationException {
		...
		List<PostgreSQLColumnMetaData> postgresColumnsOrdered = postgresqlTableMetaData.getColumnsOrdered();
		return areSciDBColumnsSameAsPostgresColumns(scidbArrayMetaData, postgresqlTableMetaData, scidbColumnsOrdered,
				postgresColumnsOrdered);
	}
	 ...
	 * @throws MigrationException
	 */
	public static boolean areSciDBColumnsSameAsPostgresColumns(SciDBArrayMetaData scidbArrayMetaData,
			PostgreSQLTableMetaData postgresqlTableMetaData, List<SciDBColumnMetaData> scidbColumnsOrdered,
			List<PostgreSQLColumnMetaData> postgresColumnsOrdered) throws MigrationException {
		if (scidbColumnsOrdered.size() == postgresColumnsOrdered.size()) {
			...
			for (int i = 0; i < scidbColumnsOrdered.size(); ++i) {
				if (!scidbColumnsOrdered.get(i).getColumnName().equals(postgresColumnsOrdered.get(i).getName())) {
					String msg = "The attribute " + postgresColumnsOrdered.get(i).getName()
							+ " from PostgreSQL's table: " + postgresqlTableMetaData.getSchemaTable().getFullName()
							+ " is not matched in the same ORDER with " + "attribute/dimension in the array in SciDB: "
							+ scidbArrayMetaData.getArrayName() + " (position " + i
							+ " PostgreSQL is for the attribute " + postgresColumnsOrdered.get(i).getName()
							+ " whereas the position " + i + " in the array in SciDB is: "
	 ...
	 * @throws SQLException
	 */
	public static void removeArray(SciDBConnectionInfo connectionTo, String arrayName) throws SQLException {
