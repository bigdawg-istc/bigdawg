...
public class SciDBDataLoader {
	...
	static Logger log = org.apache.log4j.Logger.getLogger(SciDBDataLoader.class
			.getName());
	...
	public Response load(String istream) {
		...
		try {
			requestData = mapper.readValue(istream,
		...
		try {
			InputStream inStream = executeLoading(requestData.getScript(),
	...
	private InputStream executeLoading(String script, String dataLocation,
			String flatArrayName, String arrayName) throws IOException,
			InterruptedException, SciDBLoaderException {
		Process prop = new ProcessBuilder(script, dataLocation, flatArrayName,
				arrayName).start();
		prop.waitFor();
	...
	public static void main(String[] args) {
		...
		String script = System.getProperty("user.dir")
				+ "/scripts/test_script/scidbLoad.sh";
		String dataLocation = System.getProperty("user.dir")
				+ "/scripts/test_script/";
		String flatArrayName = "myFlatArrayName";
		String arrayName = "myArrayName";
		try {
			InputStream inStream = new SciDBDataLoader().executeLoading(script,
					dataLocation, flatArrayName, arrayName);
			String result = IOUtils.toString(inStream, Constants.ENCODING);
