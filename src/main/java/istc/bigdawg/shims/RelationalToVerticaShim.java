 ...
 * @author kateyu
 */
public class RelationalToVerticaShim implements Shim {

    private VerticaConnectionInfo ci;
    private VerticaHandler handler = null;

    @Override
    public void connect(ConnectionInfo ci) throws ShimException {
        if (ci instanceof VerticaConnectionInfo) {
            this.ci = (VerticaConnectionInfo) ci;
            handler = new VerticaHandler(this.ci);
        } else {
            throw new ShimException("Input Connection Info is not VerticaConnectionInfo; rather: "+ ci.getClass().getName());
        }
    }
