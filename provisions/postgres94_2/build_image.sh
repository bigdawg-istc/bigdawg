...
if [ ! -f provisions/postgres94_2/mimic2.pgd.tar.gz ]; then
    ...
    tar -cvzf mimic2.pgd.tar.gz mimic2.pgd && mv mimic2.pgd.tar.gz provisions/postgres94_2/ && rm mimic2.pgd
else
...
cp -r src/main/resources provisions/postgres94_2/

echo "Copying the middleware jar to image build context"
cp -a target/istc.bigdawg-1.0-SNAPSHOT-jar-with-dependencies.jar provisions/postgres94_2

echo "Building postgres94_1"
docker build -t bigdawg/postgres2 provisions/postgres94_2
