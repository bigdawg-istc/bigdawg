...
echo "==> mysql-data"
docker rm -f bigdawg-mysql-data
docker run -d --net=bigdawg -h bigdawg-mysql-data -p 3306:3306 -e "BDHOST=bigdawg-mysql-data" --name bigdawg-mysql-data bigdawg/mysql
# echo "==> scidb"
...
docker exec bigdawg-postgres-catalog /bdsetup/setup_mysql.sh
...
# mysql-data
docker cp cluster_setup/mysql-data/bdsetup bigdawg-mysql-data:/home/mysql
docker exec --user=root bigdawg-mysql-data chmod +x /home/mysql/bdsetup/setup.sh
docker exec --user=mysql bigdawg-mysql-data /home/mysql/bdsetup/setup.sh
