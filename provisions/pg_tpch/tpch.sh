...
function benchmark_dss() {
	...
	mkdir $RESULTS/vm_stat-s $RESULTS/vm_stat-d $RESULTS/explain $RESULTS/results $RESULTS/errors
	...
	vm_stat -s > $RESULTS/vm_stat-s-before.log 2>&1
	vm_stat -d > $RESULTS/vm_stat-d-before.log 2>&1
	...
	do
		...
		if [ -f "$q" ]; then
			...
			vm_stat -s > $RESULTS/vm_stat-s/before-$n.log 2>&1
			vm_stat -d > $RESULTS/vm_stat-d/before-$n.log 2>&1
			...
			vm_stat -s > $RESULTS/vm_stat-s/after-$n.log 2>&1
			vm_stat -d > $RESULTS/vm_stat-d/after-$n.log 2>&1
	...
	vm_stat -s > $RESULTS/vm_stat-s-after.log 2>&1
	vm_stat -d > $RESULTS/vm_stat-d-after.log 2>&1
...
function stat_collection_start()
{
	...
	# run some basic monitoring tools (iotop, iostat, vm_stat)
	for dev in $DEVICES
	do
		iostat -t -x /dev/$dev $DELAY >> $RESULTS/iostat.$dev.log &
	done;

	vm_stat $DELAY >> $RESULTS/vm_stat.log &
